...
namespace ft
{
	...
	class map {
		...
		public :
			...
			iterator begin() 			{ return (iterator(_bst.min_node(_bst._node))); }
			// iterator end()   			{ return (iterator(nullptr, _root_node)); }
			iterator end()   			{ return _bst.end(); }
			reverse_iterator rbegin() 	{ return _bst.end(); }
			reverse_iterator rend() 	{ return (reverse_iterator(_bst.min_node(_bst.node))); }
			...
			size_type count( const Key& key ){
				size_type count = 0;
				for (iterator it = begin(); it != _bst.end()->parent; it++)
					if (it->first == key)
						count++;
				return count;
			}
